...
def fetch_updated_date_from_site():
    try:
        ...
        figcaption_element = soup.find('figcaption', string=lambda text: text and "Updated:" in text)
        ...
        if figcaption_element:
            ...
            if updated_date_str: # Garante que algo foi extraído
                ...
                logger.warning("Texto 'Updated:' encontrado, mas a data subsequente está vazia.")
                return None
        else:
...
async def check_for_update(context: CallbackContext):
    ...
    last_known_update_date = load_last_updated_date() # Esta função já está em utils/files

    if current_site_update_date:
        if last_known_update_date is None or current_site_update_date != last_known_update_date:
            logger.info(f"Nova atualização detectada! Data do site: {current_site_update_date}, Última conhecida: {last_known_update_date}")
            save_last_updated_date(current_site_update_date) # Esta função já está em utils/files

            chat_ids_to_notify = load_chat_ids() # Esta função já está em utils/files
            if not chat_ids_to_notify:
                logger.warning("Nenhum chat ID encontrado para notificar sobre a atualização.")
                return
            ...
            for chat_info in chat_ids_to_notify:
                ...
                if chat_id:
                    try:
                        ...
                        logger.info(f"Notificação de atualização enviada para o chat ID: {chat_id}")
                    except Exception as e:
                        logger.error(f"Falha ao enviar notificação de atualização para o chat ID {chat_id}: {e}")
                else:
                    logger.warning(f"Chat info sem chat_id encontrado: {chat_info}")
        else:
            logger.info(f"Nenhuma nova atualização detectada. Data do site ({current_site_update_date}) é a mesma da última conhecida.")
    else:
        logger.warning("Não foi possível obter a data de atualização do site. Nenhuma ação tomada.")
