...
def schedule_link_jobs_for_all_chats(job_queue: JobQueue):
    ...
    for chat_info in chat_ids_data:
        ...
        if chat_id:
            # Agendamento do lembrete Marvel Snap Zone (mantido)
            ...
            if not current_jobs:
                ...
                logger.info(
                    f"Job '{job_name}' agendado para o chat '{chat_name}' (ID: {chat_id})."
                )
            else:
                logger.info(
                    f"Job '{job_name}' já existe para o chat '{chat_name}' (ID: {chat_id}). Não será reagendado."
                )
